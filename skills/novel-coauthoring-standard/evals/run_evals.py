...
def run(repo_root: Path) -> int:
    matrix_path = repo_root / "skills" / "novel-coauthoring-standard" / "evals" / "executable-matrix.json"
    ...
    checks = {
        ...
        "integration.agent.retry.idempotent": ["X-Client-Id", "X-Timestamp", "X-Nonce", "X-Signature", "idempotent_replay=true", "latency_ms", "prompt_tokens", "completion_tokens"],
        "feature.scene.simulate": ["simulation", "turns", "ooc_risks", "author_edit_points", "不改写目标", "不改写结局"],
        "feature.outline.board": ["outline.vertical", "outline.horizontal", "outline.links", "risk_flags"],
    }
