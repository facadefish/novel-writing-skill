...
## 5.1 多角色场景模拟（单一入口）
```bash
novel scene simulate --event e018 --chars "linyan,hantu,zhifa,kuanggongA,kuanggongB" --scene "玄铁矿场夜审" --goal "夺钥符并立威" --ending "林砚带伤突围"
```

## 5.2 大纲看板（横纵双维）
```bash
novel outline view --book-id xh_demo_001 --scope chapter --target ch001 --view both
```
...
novel ss --event e018 --chars "linyan,hantu,zhifa,kuanggongA,kuanggongB" --scene "玄铁矿场夜审" --goal "夺钥符并立威" --ending "林砚带伤突围"
novel ov --book-id xh_demo_001 --scope chapter --target ch001 --view both
